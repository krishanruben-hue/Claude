...
async function searchCard(name, cardNumber) {
  ...
  const res = await axios.get(`https://${RAPIDAPI_HOST}/pokemon/cards/search?${params}`, {
...
const setFilter = process.argv[2]; // optional: node link-cards.js "Prismatic Evolutions"
...
for (const card of cards) {
  try {
    const results = await searchCard(card.name, card.set_number);
    if (results.length === 0) {
